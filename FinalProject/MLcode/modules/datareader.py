...
############################################################
def DataLoader(data_dir , shuffle , max_len =2000 ):
    ...
    to lower case
    '''
    tokenizer =tf.keras.preprocessing.text.Tokenizer(num_words=54376, oov_token='xxxxxxx') #10000
    ...
    #X_dict.items()
    X_seq = tokenizer.texts_to_sequences(X)
    X_padded_seq = tf.keras.preprocessing.sequence.pad_sequences(X_seq, padding='post', maxlen=max_len)  # 100)
    ...
    if shuffle:
        idx = np.random.permutation(len(db))
        X_padded_seq =X_padded_seq[idx]
        YY = YY[idx]

    #X_padded_seq.shape

    return X_padded_seq, YY, len(  X_dict.items()) , tokenizer , le
def train_val_spliter(features, labels , val_split =0.1):
    x_train, x_val, y_train, y_val = train_test_split(features, labels, test_size=0.1, shuffle=True,stratify=labels)
    # or this way:
    # train_idxs, val_idxs = stratified_split(features, labels, val_percent=0.2)
    # print(x_train[0])
    # print(y_train[0])
